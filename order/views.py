...
#for coupon code. we add coupon model and form from coupon app
...
#for cart view./ all order and cart product view . it use cart.html page. add coupon form 
def cart_view(request):
    if request.user.is_authenticated:
        ...
        return redirect('account:login')
# for delete cart item from cart index page
...
#for increase cart quantity 
def increase_cart(request, pk):
    item = get_object_or_404(Product, pk=pk)
    order_qs = Order.objects.filter(user=request.user, ordered= False)
    if order_qs.exists():
        order = order_qs[0]
        if order.orderitems.filter(item=item).exclude():
            order_item = Cart.objects.filter(item=item, user=request.user,purchased=False)[0]
